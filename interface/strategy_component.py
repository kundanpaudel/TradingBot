...
class StrategyEditor(tk.Frame):
    def __init__(self, *args, **kwargs):
        ...
        self._add_button = Button(self._commands_frame, text="Add Strategy", font=GLOBAL_FONT, command=self._add_strategy_row, 
    ...
    def _add_strategy_row(self):
        ...
        for col, base_param in enumerate(self._base_params):
            ...
            if base_param['widget'] == tk.OptionMenu:
                ...
                self.body_widgets[code_name][b_index] = Button(self._table_frame, text=base_param['text'], bg=base_param['bg'], fg=FG_COLOR, command=lambda frozen_command=base_param['command']: frozen_command(b_index))
            else:
                continue
    ...
    def _show_popup(self, b_index:int):
        ...
        ## Validation Button ##
        validation_button = Button(self._popup_window, text="Validate", bg=BG_COLOR_2, fg=FG_COLOR, command=lambda: self._validate_parameters(b_index))
